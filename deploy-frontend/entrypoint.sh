...
app_name=$(echo $app_name | sed -e 's/^agwafarm\///')
app_name=$(echo $app_name | sed -e 's/^agwa\-//')
...
export APP_BUCKET=$(${target_env}-agwa-$app_name-web)
...
cdk deploy --require-approval never $APP_STACKS --parameters Environment=$target_env --parameters Bucket=$APP_BUCKET --parameters BucketPrefix=$APP_BUCKET_PREFIX
...
if [ "$s3_retainment" = "standard" ]; then
   ...
   # copy web assets
   aws s3 sync --delete build $s3_path_base/statics
